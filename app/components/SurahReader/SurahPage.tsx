...
export function SurahPage({ page }: SurahPageProps) {
  const { pageNumber, isComplete, words } = page;

  if (!isComplete) {
    return <SurahPageShimmer linesPerPage={15} />;
  }
  const groupWordsByLine = Object.groupBy(Object.values(words), (word) => word.line_number);
  ...
  return (
    ...
    >
      <div
        className="font-kfgqpc flex flex-col text-[1.1rem] @[350px]:text-[1.1rem] @[400px]:text-[1.3rem] @[450px]:text-[1.4rem] @[500px]:text-[1.6rem] @[550px]:text-[1.8rem] @[600px]:text-[2rem] @[650px]:text-[2.25rem] @[750px]:text-[2.5rem] @[800px]:text-[3rem] leading-relaxed"
        lang="ar"
        dir="rtl"
        role="group"
      ...
      >
        {wordsMappedByLine.map(({ lineNumber, words }) => {
          return <VerseLine key={lineNumber} words={words} />;
        })}
      </div>
